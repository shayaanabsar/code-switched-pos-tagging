...
if avoid_language != '':
	...
	filtered_inputs, filtered_outputs, filtered_lang_tags = [], [], [] 
	...
	for i in range(pp.num_sequences):
		if lang_tags[i] != code:
			...
			filtered_lang_tags.append(lang_tags[i])
		else:
			hidden_inputs.append(inputs[i])
			hidden_outputs.append(outputs[i])

	o = len(filtered_inputs)
...
class Model(nn.Module):
	...
	def forward(self, input, train=True):
		...
		x = self.batch_norm(new_embeddings)
		x = self.dropout(new_embeddings)
		x = self.linear(x)
